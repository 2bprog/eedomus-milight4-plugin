Ce plugin eedomus permet de piloter les ampoules connectées à un bridge Mi-Light v4 ou v5.

## Prérequis 

* Une box eedomus+ 
* Un bridge Mi-Light v4 ou v5 installé et configuré.

Exemple de controleur Mi-Light v4 :

![Exemple Mi-Light v4](https://raw.githubusercontent.com/2bprog/eedomus-milight4-plugin/master/doc/milightv4.png)

## Installation

* Cliquez sur "Configuration" / "Ajouter ou supprimer un périphérique" / "Store eedomus" / "Milight v4" / "Créer"

## Champs a configurer : 

![Configuration Mi-Light v4 ou v5](https://raw.githubusercontent.com/2bprog/eedomus-milight4-plugin/master/doc/milight4.jpg)

* IP + Port : Adresse IP et port (sous la forme x.x.x.x:yyyy) de votre bridge Mi-Light v4 ou v5, par défaut le port est 8899 
* Type d'ampoule : Ampoule Blanche ou Ampoule RGBW
* Groupe : Le groupe sur lequel est connecté l'ampoule, vous pouvez egalement sélectionner tous les groupes.

## Périphériques crées en fonction de votre sélection : 

* [1] Ampoule blanche

![Ampoule blanche](https://raw.githubusercontent.com/2bprog/eedomus-milight4-plugin/master/doc/ws.jpg)

* [2] Ampoule RGBW

![Ampoule RGBW](https://raw.githubusercontent.com/2bprog/eedomus-milight4-plugin/master/doc/rgb-0.0.6.jpg)

## Périphériques testés

* Ampoule Mi-light RGBW 6W [2]
* Ampoule Mi-light RGBW 9W [2]

## Sources et historique des versions

* [Sources](https://github.com/2bprog/eedomus-milight4-plugin)
* [Historique des versions](https://github.com/2bprog/eedomus-milight4-plugin/blob/master/CHANGELOG.md)
